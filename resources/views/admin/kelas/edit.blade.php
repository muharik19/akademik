        ...
        <div class="row">
            <div class="col-md-12 col-sm-12">
                <div class="x_panel">
                    ...
                    <div class="x_content">
                        <form class="" action="{{ route('classe.update',$editkelas->id) }}" method="POST" autocomplete="off">
                            @method('patch')
                            @csrf
                                ...
                                <div class="col-md-6 col-sm-6">
                                    <select class="form-control @error('jurusan') is-invalid @enderror" name="jurusan" id="jurusan" required="required">
                                        ...
                                        @foreach($major as $majors)
                                            <option value="{{ $majors->id }}" {{ (isset($editkelas->jurusan_id) && $editkelas->jurusan_id === $majors->id) ? 'selected' : '' }}>{{ $majors->nama_jurusan }}</option>
                                        @endforeach
                                    </select>
                                    @error('jurusan')
                            ...
                            <div class="field item form-group">
                                ...
                                <div class="col-md-6 col-sm-6">
                                    <input class="form-control @error('nama_kelas') is-invalid @enderror" type="text" name="nama_kelas" id="nama_kelas" value="{{ $editkelas->nama_kelas }}" placeholder="Nama Kelas" required="required" />
                                    @error('nama_kelas')
                                        <span class="badge badge-warning">{{ $message }}</span>
                                    @enderror
                                </div>
                            ...
                            <div class="field item form-group">
                                ...
                                <div class="col-md-6 col-sm-6">
                                    <div class="radio col-md-2 col-sm-2">
                                        <label for="yes">
                                            <input type="radio" class="@error('aktif') is-invalid @enderror" name="aktif" {{ (isset($editkelas->aktif) && $editkelas->aktif === 'Y') ? 'checked' : '' }} id="yes" value="Y" required="required"> Yes
                                        </label>
                                    </div>
                                    <div class="radio col-md-2 col-sm-2">&nbsp;&nbsp;&nbsp;&nbsp;
                                        <label for="no">
                                            <input type="radio" class="@error('aktif') is-invalid @enderror" name="aktif" {{ (isset($editkelas->aktif) && $editkelas->aktif === 'N') ? 'checked' : '' }} id="no" value="N" required="required"> No
                                        </label>
                                    </div>
                                </div>
                            </div>
